...
<head>
    ...
    <style>
        ...
        .section-card h2 { color: var(--accent-color); border-bottom: 2px solid var(--border-color); padding-bottom: 12px; font-size: 20px; display: flex; justify-content: space-between; align-items: center; }
        pre { background-color: var(--code-bg); padding: 18px; border-radius: 6px; border: 1px solid var(--border-color); color: var(--code-text); font-family: 'JetBrains Mono'; white-space: pre-wrap; }
        .card-copy-btn { background-color: rgba(52, 211, 153, 0.15); border: 1px solid var(--code-text); color: var(--code-text); padding: 6px 12px; border-radius: 4px; cursor: pointer; }
    </style>
</head>
<body>
    ...
    <div id="login-overlay">
        <div class="login-box">
            <h1>SİSTEM ERİŞİMİ</h1>
            <p>TACTICAL MEDIC DATABASE</p>
            <input type="password" id="password-field" placeholder="Erişim Şifresi" onkeydown="if(event.key === 'Enter') checkPassword()">
            <button onclick="checkPassword()">YETKİLENDİR</button>
            <div id="error-msg">⚠️ GEÇERSİZ ŞİFRE!</div>
        </div>
    </div>

    <div id="main-app">
        <aside class="sidebar" id="nav-list">
            <div style="padding: 25px; text-align: center; border-bottom: 2px solid #334155;">
                <h3 style="color:var(--accent-color)">USMS ACADEMY</h3>
            </div>
            <div style="padding: 10px;">
                <div class="menu-item" style="padding: 12px; color:#9ca3af; cursor:pointer;" onclick="window.scrollTo(0, document.getElementById('sec1').offsetTop)">1. Olay Yeri Giriş</div>
                <div class="menu-item" style="padding: 12px; color:#9ca3af; cursor:pointer;" onclick="window.scrollTo(0, document.getElementById('sec31').offsetTop)">31. Klinik Ölüm</div>
            </div>
        </aside>
...
/me Ölüm saati: [SAAT] olarak not düşer.</pre>
    ...
    <script>
        // ŞİFRE GÜNCELLENDİ: musa32
        const CORRECT_PASSWORD = "musa32"; 

        function checkPassword() {
            const inputField = document.getElementById("password-field");
            if (inputField.value === CORRECT_PASSWORD) {
                document.getElementById("login-overlay").style.opacity = "0";
                setTimeout(() => { document.getElementById("login-overlay").style.display = "none"; document.getElementById("main-app").style.display = "grid"; }, 500);
            } else {
